...
// player id
let globalPID = 0;
// source id is 1028 for favorites
let globalSID = 1028;
// station list
let globalStations = [];
...
h.on("response", res => {
  ...
  if (res.heos.command === "browse/browse" && res.hasOwnProperty("payload")) {
    globalStations = res.payload;
    console.log("got stations: ", globalStations);
  }
});
...
h.on("ready", _ => {
  ...
  // browse favorites
  h.request("browse/browse?sid=" + globalSID);
});
...
const processBaosValue = payload => {
  ...
  switch (id) {
    case DP_PREVNEXT:
      ...
      cmd += "?pid=" + globalPID;
      cmd += "&sid=" + globalSID;
      cmd += "&mid=" + globalStations[globalStationIndex].mid;
      h.request(cmd);
      break;
    case DP_PLAYSTOP:
      // send play req
